...
| `clamp` | Apply deterministic fixed-point sigmoid clamp to reputation values; the pipeline clamp skips CarryForward entries so an already-finalized reputation is not decayed | `ReputationVector`, previous and contribution vectors, `PorConfig` | clamped `ReputationVector` | `config`, `types`, `error` | `clamp_reputation_value`, `clamp_reputation_vector`, `clamp_reputation_transition` |
...
7. The next vector is clamped with `clamp_reputation_transition`, which applies the sigmoid to rated and newly seeded nodes and leaves CarryForward entries at their previous finalized reputation. The sigmoid is not idempotent, so clamping those entries would decay them every sparse round. This is a pure calculation and does not mutate state.
